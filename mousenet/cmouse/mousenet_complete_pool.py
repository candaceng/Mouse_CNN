...
from mousenet.example.config import  INPUT_SIZE, EDGE_Z, OUTPUT_AREAS, HIDDEN_LINEAR, NUM_CLASSES, get_resolution, INPUT_CORNER, SUBFIELDS
        ...
        for area in self.top_sort:
            ...
            if area == 'LGNd' or area == 'LGNv':
                ...
                if SUBFIELDS:
                    ...
                    source_field = torch.narrow(torch.narrow(x, 3, left, width), 2, bottom, height) #TODO: check top/bottom direction
...
def get_subsample_indices(layer):
    ...
    input_field = [INPUT_CORNER[0], INPUT_CORNER[1],  INPUT_CORNER[0] + image_width,INPUT_CORNER[1]+image_height]
    ...
    if source_area != "LGNd" and source_area != "input":
        ...
        padding_right = right+right_extra-source_right if right+right_extra >=source_right  else 0
        ...
        padding_top = top+top_extra-source_top if top+top_extra >=source_top  else 0
