...
parser.add_argument(
    "-data", metavar="DIR", help="path to dataset", default=IMAGENET_DIR
)
parser.add_argument(
    "-j",
    "--workers",
...
parser.add_argument(
    "--momentum", default=MOMENTUM, type=float, metavar="M", help="momentum"
)
parser.add_argument(
    "--wd",
    "--weight-decay",
...
parser.add_argument(
    "--seed", default=2020, type=int, help="seed for initializing training. "
)
...
def main():
    if USE_WANDB:
        ...
        config.log_interval = (
            100  # how many batches to wait before logging training status
        )
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if NET == 1:
        net_name = "network_complete_updated_number(%s,%s,%s)" % (
            INPUT_SIZE[0],
            INPUT_SIZE[1],
            INPUT_SIZE[2],
        )
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if USE_WANDB:
            ...
            if (
                i % config.log_interval == config.log_interval - 1
...
def save_checkpoint(state, is_best):
    ...
    filename = (
        ...
        + RUN_NAME_MASK
        + "/%s_%s_%s_%s.pth.tar"
        % (FIXMASK, args.seed, state["epoch"], state["best_acc1"])
    )
    ...
    if is_best:
        shutil.copyfile(
            ...
            + "/"
            + RUN_NAME_MASK
            + "/%s_%s_model_best.pth.tar" % (FIXMASK, args.seed),
        )
